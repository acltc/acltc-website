...
before_action :authenticate_admin!, except: [:interview_options]
  ...
  def interview_options
    @interviews = Interview.where('starts_at > ? AND starts_at < ?', DateTime.now, DateTime.now + 72.hours).where(booked: false).where("location = '" + params[:id] + "' OR location IS NULL").order(starts_at: :asc)
